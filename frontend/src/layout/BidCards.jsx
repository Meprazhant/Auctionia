...
function BidCards({ bid, user }) {
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl">
      <figure>
        <img className="h-96 w-full object-cover" src={bid.image} alt="Shoes" />
      </figure>
      <div className="card-body">
        <h2 className="card-title">{bid.name}</h2>
        <p>{bid.description.substring(0, 50)} ...</p>
        <div className="flex gap-2 flex-wrap flex-col">
          ...
          <div className="badge badge-primary">
            Total Bids: {bid?.totalBids.length}
          </div>
          <div className="badge badge-warning">Category: {bid?.category}</div>
        </div>
