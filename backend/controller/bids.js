...
export const editBid = async (req, res) => {
  try {
    const bid = await bids.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    res.json(bid);
  } catch (error) {
...
export const makeBid = async (req, res) => {
  try {
    ...
    const alreadyBid = bid.totalBids.filter(
      (b) => b.user[0]._id.toString() === user._id.toString()
    );
...
export const getAllBidsOfUser = async (req, res) => {
  try {
    const bidsData = await bids.find();
    const userBids = bidsData.filter(
      (b) => b.userID.toString() === req.params.id.toString()
    );
    res.json(userBids);
  } catch (error) {
    res.status(400).json({ message: error.message });
...
//get all bids that user had bidden
export const getAllBidsOfUserBidden = async (req, res) => {
  try {
    const bidsData = await bids.find();
    const userBids = bidsData.filter((b) =>
      b.totalBids.some((t) => t.user[0]._id.toString() === req.params.id)
    );
    res.json(userBids);
  } catch (error) {
    res.status(400).json({ message: error.message });
